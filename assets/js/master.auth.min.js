...
var  $reactivate_modal = $("#reactivate-modal"),
     $reactivate_form = $("#reactivate-form")
     $pass_reactivate = $("#master-auth-password-reactivate");

$(document).on("click", ".send-reactivate-link", function (e) {
    e.preventDefault();
    $reactivate_form.attr("action", $(this).data("url"));
    $reactivate_modal.modal("show");
    $pass_reactivate.val("");
    $(".btn-proceed-reactivate").html("Proceed").prop("disabled", false);
});

$(document).on("click", ".btn-proceed-reactivate", function (e) {
    e.preventDefault();
    console.log( $reactivate_form.attr("action"));
    var $this = $(this);
    var random = $this.data('random')
    if ($pass_reactivate.val() == "") {
        notify_bar("danger", "Enter Your Master Password");
        return false;
    }
    $.ajax({
        url: $reactivate_form.attr("action"),
        type: "PUT",
        data: $reactivate_form.serialize(),
        async: true,
        beforeSend: function () {
            $this.html("Loading...").prop("disabled", true);
        },
        success: function (response) {
            $pass_reactivate.val("").html("Proceed").prop("disabled", false);
            if (response.status) {
                if (response.link) {
                    copyToClipboard(response.link);
                }
