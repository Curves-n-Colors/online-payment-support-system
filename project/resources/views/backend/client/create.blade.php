        ...
        <div class="col-sm-12 col-md-12 col-lg-5 col-xlg-4">
            ...
            <form role="form" method="POST" action="{{ route('client.store') }}">
                ...
                <div class="form-group required form-group-default @error('phone_no') has-error @enderror">
                    <label>Phone</label>
                    <div class="controls">
                        <input type="text" class="form-control @error('phone_no') error @enderror" name="phone_no" placeholder="Phone" required autocomplete="off" autofocus value="{{ old('phone_no') }}">
                        @error('phone_no')
                            <label class="error">{{ $message }}</label>
                        @enderror
                    </div>
